 ...
 */
final class Version20241108101135 extends AbstractMigration
{
    ...
    public function up(Schema $schema): void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__event AS SELECT id, project_id, name, description, accessible, prerequisites, start_at, end_at, archived FROM event');
        ...
        , archived BOOLEAN DEFAULT NULL, CONSTRAINT FK_3BAE0AA7166D1F9C FOREIGN KEY (project_id) REFERENCES project (id) ON UPDATE NO ACTION ON DELETE NO ACTION NOT DEFERRABLE INITIALLY IMMEDIATE, CONSTRAINT FK_3BAE0AA7B03A8386 FOREIGN KEY (created_by_id) REFERENCES user (id) NOT DEFERRABLE INITIALLY IMMEDIATE)');
        $this->addSql('INSERT INTO event (id, project_id, name, description, accessible, prerequisites, start_at, end_at, archived) SELECT id, project_id, name, description, accessible, prerequisites, start_at, end_at, archived FROM __temp__event');
    ...
    public function down(Schema $schema): void
    {
        ...
        , end_at DATETIME NOT NULL --(DC2Type:datetime_immutable)
        , archived BOOLEAN DEFAULT NULL, CONSTRAINT FK_3BAE0AA7166D1F9C FOREIGN KEY (project_id) REFERENCES project (id) NOT DEFERRABLE INITIALLY IMMEDIATE)');
        $this->addSql('INSERT INTO event (id, project_id, name, description, accessible, prerequisites, start_at, end_at, archived) SELECT id, project_id, name, description, accessible, prerequisites, start_at, end_at, archived FROM __temp__event');
        $this->addSql('DROP TABLE __temp__event');
        $this->addSql('CREATE INDEX IDX_3BAE0AA7166D1F9C ON event (project_id)');
        $this->addSql('CREATE TEMPORARY TABLE __temp__project AS SELECT id, name, summary, created_at, updated_at FROM project');
